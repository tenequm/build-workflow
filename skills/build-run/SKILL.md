...
1. Preflight: `bernstein doctor`; `bernstein adapters list` shows herdr-claude, herdr-codex, herdr-agy; `bernstein.yaml` from `templates/bernstein.yaml` names the gates; `<run>/readiness/pins.json` exists and `bernstein-herdr ready` is clean; the repo root is checked out on the integration branch, never main.
2. Layout: `herdr workspace create --cwd <repo root> --label build-<slug> --no-focus`; write its ids to `<run>/herdr.json` (`{"workspace": ..., "root_pane": ...}`); run `bernstein live` in the root pane. The adapter opens one tab per executor, judge and shadow lane in that workspace and never touches the driver's own pane.
