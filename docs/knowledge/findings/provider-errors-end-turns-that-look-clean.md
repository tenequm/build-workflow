...
description: Gemini backend 429s (burst-limit RESOURCE_EXHAUSTED) terminated review sessions mid-task, yet every such turn ended with a normal end_turn and process exit 0 - the runner therefore inspects the stream's terminal output for provider-error signatures and fails the session, because a lens that could not run is not a lens with no findings.
