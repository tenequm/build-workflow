...
The stop alone is not the boundary - the driver's contract is. The
scheduler's exit is a heuristic (`run_completed` precedes finalization; the
watchdog's stand-down check has a bug that can restart a finished spawner;
residual child processes are measured, not stopped), the task server and
watchdog outlive the scheduler, and ingress paths (workflow importer,
