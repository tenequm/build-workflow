...
sources:
  ...
  - id: carriers
    resource: "The five current lens role prompts and /skills/review-pr/templates/bernstein-templates/roles/report-writer/system_prompt.md: lenses require the bare subject name; the report contract requires identifier and requires claim to name it"
    title: where the rule now lives
  - id: split
    resource: "Git commit e45ffbb: shared finding rules moved from review-goal.md into each lens role template; the report schema remained in the report-writer role"
...
The rule first went into the then-monolithic goal text as a prose requirement -
the name belongs in `claim` or `evidence`, the author's choice which. The next
run failed anyway, and the way it failed is the whole finding.[^third]
...
The third is to raise the reviewer rather than lower the bar: state the requirement
in the model-facing finding contract and let the case be passed on merit or failed
honestly. It originally landed in the goal; after the prompt split the five lens
role templates own subject naming and the report-writer role owns the `identifier`
schema.[^split][^carriers] The original change cost a full four-case re-proof because
it changed the goal text - the price of the rule being real at that time.
...
[^carriers]: the current lens and report-writer contracts
[^split]: the prompt split that moved the shared finding rule out of the goal
