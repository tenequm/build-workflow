...
* [Bernstein releases dependents on worker-reported DONE, before any verification](findings/bernstein-done-releases-before-verification.md) - the engine's dependency model is optimistic (gates are compensating controls, not barriers), and a response-cache hit can complete a task with no agent and no gate at all; strict verified-before-start ordering requires keeping downstream work off the task server entirely.
