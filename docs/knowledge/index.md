...
* [Every step's completion signal names the report the step must commit](decisions/completion-signals-name-the-report.md) - the report is what the scorer archives and the judge reads, so a step whose signals name only its code passes with the report missing and parks a later phase on an out-of-scope finding.
...
* [An executor model below the brief's complexity fails as a clean exit](findings/executor-model-capability-floor.md) - gpt-5.6-luna wandered off the same brief gpt-5.6-terra and Claude Sonnet 5 completed, exiting 0 with nothing committed, which reads as a sandbox fault while the cause is capability; role_model_policy is a correctness property.
